...
import { WorkflowNew, WorkflowHdr, sequelize, InboundMessages, Customer, Contact, Chat, Interaction, Helpdesk, BusinessEntity, BusinessUnit, InteractionTxn, Kiosk, WorkflowMapping, Address, AddressLookup, WorkflowTxn, ChatResponse, Otp, BoosterPlans, Plan, User, BoosterPurchase } from '../model'
...
export class ChatService {
  ...
  async getAgentInfo(req, res) {
    try {
      ...
      return res.status(error.statusCode || defaultStatusCode.ERROR).send({
        message: 'Error while fetching Agent Info'
      })
  ...
  async getChatInfo(req, res) {
    try {
      ...
      return res.status(error.statusCode || defaultStatusCode.ERROR).send({
        message: 'Error while fetching Chat Info'
      })
  ...
  async addBalance(req, res) {
    ...
    try {
      ...
      const boosterPayload = {
        ...
        customerName: boosterPurchasePayload && boosterPurchasePayload.filter((ele) => ele?.name == 'CustomerName') && boosterPurchasePayload.filter((ele) => ele?.name == 'CustomerName')[0].value?.stringValue || '',
        contactNo: accessNumber,
        emailId: boosterPurchasePayload && (boosterPurchasePayload.filter((ele) => ele?.name == 'PrimaryContactEmailAddress')) && (boosterPurchasePayload.filter((ele) => ele?.name == 'PrimaryContactEmailAddress')[0].value?.stringValue).split(';')[0] || '',
  ...
  async validateOtp(req, res) {
    try {
      ...
      logger.error('----ERROR--------->', error, defaultMessage.ERROR);
      return res.status(error.statusCode || defaultStatusCode.ERROR).send({
        message: 'Error while verifying otp'
      })
      // return this.responseHelper.onError(res, new Error('Error while verifying otp'))
  ...
  async updateLanguage(req, res) {
    ...
    try {
      ...
      return res.status(error.statusCode || defaultStatusCode.ERROR).send({
        message: 'Error while updating Customer Language'
      })
  ...
  async updateBotChatSocket(req, res) {
    ...
    try {
      ...
      // console.log('body.socketId.........', body.socketId)
      await Chat.update({ socketId: body.socketId, status: 'NEW', userId: userId, startAt: new Date() }, { where: { chatId: id }, transaction: t })
      await t.commit()
  ...
  async customerInteractions(req, res) {
    try {
      ...
      return res.status(error.statusCode || defaultStatusCode.ERROR).send({
        message: 'Error while fetching interaction data'
      })
  ...
  async opendHelpdeskTickets(req, res) {
    try {
      ...
      return res.status(error.statusCode || defaultStatusCode.ERROR).send({
        message: 'Error while fetching opendHelpdeskTickets data'
      })
  ...
  async closedHelpdeskTickets(req, res) {
    try {
      ...
      return res.status(error.statusCode || defaultStatusCode.ERROR).send({
        message: 'Error while fetching closedHelpdeskTickets data'
      })
  ...
  async updateHelpdeskTicket(req, res) {
    ...
    try {
      ...
      return res.status(error.statusCode || defaultStatusCode.ERROR).send({
        message: 'Failed to updated helpdesk Ticket'
      })
  ...
  async humanHandover(req, res) {
    ...
    try {
      ...
      if (!helpdeskData) {
        ...
        await t.commit();
        return res.status(error.statusCode || defaultStatusCode.ERROR).send({
          message: 'Error while creating helpdesk record'
        })
      ...
      return res.status(error.statusCode || defaultStatusCode.ERROR).send({
        message: 'Failed to create human handover chat'
      })
  ...
  async chatByWorkflow(req, res) {
    try {
      ...
      return res.status(error.statusCode || defaultStatusCode.ERROR).send({
        message: 'Error while creating new chat user'
      })
  ...
  async updateChat(req, res) {
    ...
    try {
      ...
      return res.json('Error while updating Chat');
    } finally {
      if (t && !t.finished) {
        await t.rollback()
      }
  ...
  async chatCleanUp(req, res) {
    ...
    try {
      ...
      logger.error(error, defaultMessage.ERROR);
      return res.status(error.statusCode || defaultStatusCode.ERROR).send({
        message: 'Error while Cleaning the chat'
      })
  ...
  async getServiceTypes(req, res) {
    try {
      ...
      return res.status(error.statusCode || defaultStatusCode.ERROR).send({
        message: 'Error while fetching Service Types'
      })
  ...
  async getTarrifName(req, res) {
    try {
      ...
      return res.status(error.statusCode || defaultStatusCode.ERROR).send({
        message: 'Error while fetching Tarrif Name'
      })
  ...
  async getBoosterPlans(req, res) {
    try {
      ...
      return res.status(error.statusCode || defaultStatusCode.ERROR).send({
        message: 'Error while fetching booster plans'
      })
  ...
  async getImagineGo(req, res) {
    try {
      ...
      logger.error(error, 'Error while fetching enquirys');
      return res.status(error.statusCode || defaultStatusCode.ERROR).send({
        message: 'Error while fetching enquirys'
      })
      // return this.responseHelper.onError(res, new Error('Error while fetching enquirys'))
    }
  }
